...
const ProfilePage = () => {
  ...
  const getStatusTag = (status) => {
    ...
    return <Tag icon={s.icon} color={s.color} className="status-tag m-0">{s.text.toUpperCase()}</Tag>;
  };
      ...
      <div className="container">
        ...
        <div className="profile-content-grid">
          ...
          >
            {/* TAB: THÔNG TIN */}
            <Tabs.TabPane 
              tab={<span><UserOutlined /> {!isMobile && "TÀI KHOẢN"}</span>} 
              key="info"
            >
              <div className="glass-panel p-3 p-md-4">
                ...
                <Form layout="vertical" className="dark-form">
                  ...
                  <Button className="btn-accent w-100-mobile" onClick={() => message.success("Đã cập nhật!")}>LƯU THAY ĐỔI</Button>
                </Form>
              </div>
            </Tabs.TabPane>
            ...
            {/* TAB: ĐƠN HÀNG */}
            <Tabs.TabPane 
              tab={<span><ShoppingOutlined /> {!isMobile && "ĐƠN HÀNG"}</span>} 
              key="orders"
            >
              <div className="glass-panel p-3 p-md-4">
                  ...
                  <div className="order-list-scroll">
                    {orders.map(o => (
                      <div key={o.id} className="order-item-card mb-3">
                        ...
                        {o.items.map(item => (
                          <div key={item.id} className="d-flex gap-2 gap-md-3 mb-3 item-row">
                            <img src={IMAGE_URL + item.image} className="order-img" alt="" />
                            <div className="flex-grow-1 min-width-0">
                              <div className="text-white fw-bold text-truncate">{item.product_name}</div>
                              <div className="text-secondary small">x{item.quantity} - {Number(item.price).toLocaleString()}đ</div>
                            </div>
                            <div className="text-white fw-bold d-none d-md-block">{Number(item.total).toLocaleString()}đ</div>
                          </div>
                        ))}
                        <div className="order-footer pt-3 mt-2 border-top border-secondary d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-3">
                          <div className="shipping-info">
                             <div className="small text-secondary text-truncate"><PhoneOutlined className="me-1"/> {o.phone}</div>
                             <div className="small text-secondary text-truncate"><EnvironmentOutlined className="me-1"/> {o.address}</div>
                          </div>
            ...
            {/* TAB: BẢO MẬT */}
            <Tabs.TabPane 
              tab={<span><LockOutlined /> {!isMobile && "BẢO MẬT"}</span>} 
              key="password"
            >
              <div className="glass-panel p-3 p-md-4">
                ...
                <Form layout="vertical" className="dark-form">
                  ...
                  <Button className="btn-accent-outline w-100-mobile">CẬP NHẬT</Button>
                </Form>
              </div>
            </Tabs.TabPane>
          </Tabs>
